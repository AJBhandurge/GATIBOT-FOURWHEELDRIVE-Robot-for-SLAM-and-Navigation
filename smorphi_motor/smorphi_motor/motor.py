...
class SmorphiDriver(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('ticks_per_rev',  292.5)   # calibrated
    ...
    def _cmd_cb(self, msg: Twist):
        ...
        # ── Mecanum inverse kinematics ─────────────────────────
        #
        # Wheel layout:   M1(FL)╲  ╱M2(FR)
        #                 M4(RL)╱  ╲M3(RR)
        #
        # For standard mecanum with rollers at 45°:
        #   wFL = (vx - vy - wz*wd) / r    (\) wheel, left-front
        #   wFR = (vx + vy + wz*wd) / r    (/) wheel, right-front
        #   wRR = (vx - vy + wz*wd) / r    (\) wheel, right-rear
        #   wRL = (vx + vy - wz*wd) / r    (/) wheel, left-rear
        #
        # Sign check (vx=1, vy=0, wz=0 → all positive → all FORWARD ✓)
        # Sign check (vy=1, vx=0, wz=0 → FL-,FR+,RR-,RL+ → strafe right... 
        #   wait: FL=(\) must go backward for strafe RIGHT → correct ✓)
        # Sign check (wz=1 CCW, vx=0, vy=0 → FL-,FR+,RR+,RL- → correct ✓)
        #
